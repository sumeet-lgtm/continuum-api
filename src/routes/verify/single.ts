  ...
  properties: {
    ...
    checks: {
      ...
      properties: {
        ...
        spfValid:      { type: 'boolean' },
        dmarcValid:    { type: 'boolean' },
        dkimFound:     { type: 'boolean' },
        blacklisted:   { type: 'boolean' },
        blacklists:    { type: 'array', items: { type: 'string' } },
      },
    },
    ...
    async (request: FastifyRequest<VerifyRoute>, reply: FastifyReply) => {
      ...
      return reply.status(200).send({
        ...
        checks: {
          ...
          spfValid:      result.checks.spfValid ?? false,
          dmarcValid:    result.checks.dmarcValid ?? false,
          dkimFound:     result.checks.dkimFound ?? false,
          blacklisted:   result.checks.blacklisted ?? false,
          blacklists:    result.checks.blacklists ?? [],
        },
        score:      result.score,
