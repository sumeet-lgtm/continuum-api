  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (members.length === 0) {
        return reply
          .status(200)
          .send({ total: 0, active: 0, at_risk: 0, inactive: 0, never_opened: 0, contacts: [] });
      }
      ...
      // engagement bucket.
      const recentEvents = await withTenant(apiKeyId, (tx) =>
        tx.trackingEvent.groupBy({
          by: ['email'],
          where: {
            email: { in: emails },
            type: { in: ['open', 'click'] },
      ...
      const contacts = members.map((m) => {
        ...
        return {
          ...
          lastEngagedAt: lastAt?.toISOString() ?? null,
          bucket,
        };
      });
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (toSuppress.length === 0) {
        return reply
          .status(200)
          .send({ suppressed: 0, message: 'No contacts matched the selected buckets.' });
      }
      ...
      // withRlsBypass, not withTenant: Suppression is deliberately global (see
      // lib/bounceHandling.ts's suppress()) — upserting on its globally-unique
      // email column under a withTenant(apiKeyId) scope would make an existing
      // row owned by a different tenant RLS-invisible, and Prisma would then
      // try to INSERT instead of no-op, crashing on the unique constraint.
      ...
      return reply.status(200).send({
        ...
        message: `${toSuppress.length} contact(s) suppressed.`,
      });
    },
  );
}
