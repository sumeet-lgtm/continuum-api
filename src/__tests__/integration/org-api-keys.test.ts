...
vi.mock('../../lib/prisma.js', () => {
  ...
  // The api-keys routes go through withOrgTenant, which calls
  // prisma.$transaction(fn) and hands fn the tx — here the same mock
  // object, so tx.apiKey/tx.auditLog resolve to the mocks above.
...
vi.mock('@workos-inc/node', () => ({
  WorkOS: vi.fn().mockImplementation(function FakeWorkOS(this: object) {
    /* not used by these two endpoints */
  }),
}));
...
describe('GET /org/api-keys', () => {
  it("lists only keys tagged with the admin's own org", async () => {
    ...
    expect(mockFindManyKeys).toHaveBeenCalledWith(
  ...
  it('rejects a non-admin org member', async () => {
    const res = await app.inject({
      method: 'GET',
      url: '/org/api-keys',
      headers: await adminAuth('member'),
    });
...
describe('DELETE /org/api-keys/:id', () => {
  it("revokes a key that belongs to the admin's org", async () => {
    ...
    const res = await app.inject({
      method: 'DELETE',
      url: '/org/api-keys/key-1',
      headers: await adminAuth(),
    });

    expect(res.statusCode).toBe(200);
    expect(mockFindFirstKey).toHaveBeenCalledWith({
      where: { id: 'key-1', orgId: 'org-1' },
    ...
    expect(mockAuditCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          action: 'api_key.revoked_by_org_admin',
          orgId: 'org-1',
          apiKeyId: 'key-1',
        }),
      }),
  ...
  it('returns 404 without revoking when the key belongs to a different org', async () => {
    ...
    const res = await app.inject({
      method: 'DELETE',
      url: '/org/api-keys/someone-elses-key',
      headers: await adminAuth(),
    });
