...
export async function processCampaign(job: Job<CampaignJobData>): Promise<void> {
  ...
  if (!isSendTransportConfigured()) {
    await withTenant(apiKeyId, (tx) =>
      tx.campaign.update({
        ...
        data: {
          status: 'failed',
          errorMessage: 'Email sending is not configured. Contact support.',
        },
      }),
    );
  ...
  const campaign = await withTenant(apiKeyId, (tx) =>
  ...
  const apiKeyRecord = await withTenant(apiKeyId, (tx) =>
  ...
  const retargetExcludeSet = new Set<string>((campaign.excludedEmails as string[] | null) ?? []);

  await withTenant(apiKeyId, (tx) =>
    tx.campaign.update({ where: { id: campaignId }, data: { status: 'sending' } }),
  );
    ...
    await withTenant(apiKeyId, (tx) =>
      tx.campaign.update({
        ...
        data: {
          status: 'failed',
          errorMessage:
            'No list_ids or segment_ids specified — refusing to send to an unbounded audience.',
        },
      }),
    );
  ...
  const memberships = await withTenant(apiKeyId, (tx) =>
    tx.contactListMembership.findMany({
      ...
      include: {
        contact: { select: { email: true, firstName: true, lastName: true, customFields: true } },
      },
    }),
  );
    ...
    await withTenant(apiKeyId, async (tx) => {
      ...
      for (const seg of segments) {
        ...
        const segMemberships = await tx.contactListMembership.findMany({
          ...
          include: {
            contact: {
              select: { email: true, firstName: true, lastName: true, customFields: true },
            },
          },
        });
  ...
  const suppressions = await withRlsBypass((tx) =>
  ...
  const validRecipients = recipients.filter(
    (r) => !suppressedSet.has(r.email) && !retargetExcludeSet.has(r.email),
  );
  ...
  if (validRecipients.length > 0) {
    const ownedCount = await withTenant(apiKeyId, (tx) =>
      tx.contact.count({
  ...
  await withTenant(apiKeyId, (tx) =>
    tx.campaignRecipient.createMany({
  ...
  await withTenant(apiKeyId, (tx) =>
    tx.campaign.update({
      where: { id: campaignId },
      data: { totalRecipients: validRecipients.length },
    }),
  );
  ...
  for (let i = 0; i < validRecipients.length; i += CHUNK_SIZE) {
    ...
    const current = await withTenant(apiKeyId, (tx) =>
      tx.campaign.findUnique({
        where: { id: campaignId },
        select: { status: true, bounceCount: true, sentCount: true },
      }),
    );
    ...
    if (current && current.sentCount >= 50) {
      ...
      if (liveBouncePct > 0.05) {
        await withTenant(apiKeyId, (tx) =>
          tx.campaign.update({ where: { id: campaignId }, data: { status: 'paused_bounce' } }),
        );
        logger.warn(
          { campaignId, bouncePct: liveBouncePct },
          'Campaign auto-paused: bounce rate exceeded 5%',
        );
    ...
    const apiKeyRow = await withTenant(apiKeyId, (tx) =>
      tx.apiKey.findUnique({
        where: { id: apiKeyId },
    ...
    if (sendRemaining <= 0) {
      await withTenant(apiKeyId, (tx) =>
        tx.campaign.update({ where: { id: campaignId }, data: { status: 'paused_quota' } }),
      );
    ...
    if (i > 0) {
      ...
      const freshlySuppressed = await withRlsBypass((tx) =>
        tx.suppression.findMany({
    ...
    await Promise.allSettled(
      chunk.map(async (recipient) => {
        try {
          ...
          const rawSubject =
            isABTest && recipientVariant === 'b' && campaign.subjectB
              ? campaign.subjectB
              : campaign.subject;
          ...
          await withTenant(apiKeyId, (tx) =>
            ...
            logger.warn(
              { err, email: recipient.email, campaignId },
              'Failed to register campaign send for bounce tracking (non-fatal)',
            );
          });
          ...
          logger.error(
            { err, email: recipient.email, campaignId },
            'Campaign send to recipient failed',
          );
          await withTenant(apiKeyId, (tx) =>
            tx.campaignRecipient.updateMany({
              where: { campaignId, email: recipient.email },
    ...
    // Update progress
    await withTenant(apiKeyId, (tx) =>
      tx.campaign.update({ where: { id: campaignId }, data: { sentCount } }),
    );
    ...
    if (i + CHUNK_SIZE < validRecipients.length) {
      ...
      const delayMs = rate && rate > 0 ? Math.max(100, (CHUNK_SIZE / rate) * 3_600_000) : 100;
      ...
      if (sendDays && sendDays.length > 0) {
        while (!isInSendWindow(campaign)) {
          logger.info(
            ...
            'Campaign outside send window — waiting 15min',
          );
          await new Promise((r) => setTimeout(r, 15 * 60 * 1000));
          // Re-check cancellation
          const recheck = await withTenant(apiKeyId, (tx) =>
            tx.campaign.findUnique({ where: { id: campaignId }, select: { status: true } }),
          );
  ...
  if (stoppedEarly) {
    logger.info(
      { campaignId, sentCount, total: validRecipients.length },
      'Campaign send stopped early (cancelled/paused) — leaving status as set above',
    );
    return;
  }

  const finalCampaign = await withTenant(apiKeyId, (tx) =>
    tx.campaign.update({
      where: { id: campaignId },
...
function matchSegmentRules(
  ...
  return rules.every((rule) => {
    ...
    switch (rule.operator) {
      ...
      case 'starts_with':
        return fieldVal.toLowerCase().startsWith(rule.value.toLowerCase());
      default:
        return true;
    }
  });
